...
def parse_args():
    ...
    parser.add_argument(
        "--conditioning_image_column", type=str, default="conditioning_image", help="The column of the dataset containing an conditioning image."
    )
    parser.add_argument(
        "--caption_column",
        type=str,
...
def main():
    ...
    if args.conditioning_image_column is None:
        conditioning_image_column = dataset_columns[2] if dataset_columns is not None else column_names[2]
    else:
        conditioning_image_column = args.conditioning_image_column
        if conditioning_image_column not in column_names:
            raise ValueError(
                f"--conditioning_image_column' value '{args.conditioning_image_column}' needs to be one of: {', '.join(column_names)}"
            )
    ...
    def preprocess_train(examples):
        ...
        examples["conditioning_pixel_values"] = [train_transforms(image) for image in conditioning_images]
    ...
    def collate_fn(examples):
        ...
        return {"pixel_values": pixel_values, "conditioning_pixel_values":conditioning_pixel_values, "input_ids": input_ids}
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                conditioning_latents = vae.encode(batch["conditioning_pixel_values"].to(weight_dtype)).latent_dist.sample()
                conditioning_latents = conditioning_latents * 0.18215
